...
class CascadeRCNN(M.Module):
    ...
    def recover_pred_boxes(self, rcnn_rois, prob, nhead):
        ...
        bid = F.broadcast_to(F.expand_dims(rois[:, :1], axis=1), (m, bbox_pred.shape[1], 1))
    ...
    def compute_emd_loss_opr(self, a, b, bbox_targets, labels):
        ...
        c = a.shape[1]
        prob = F.stack([a, b], dim=1).reshape(-1, c)
        ...
        vlabel = 1 - ((labels < 0).reshape(-1, 2).sum(axis=1) > 1).float()
        loss = (cls_loss + 2 * reg_loss).reshape(-1, 2).sum(dim=1) * vlabel
        return loss
...
class RCNN(M.Module):
    def __init__(self, iou_thrs = [config.fg_threshold, 0.6], nheads = [1, 1]):
        super().__init__()
        # roi head
