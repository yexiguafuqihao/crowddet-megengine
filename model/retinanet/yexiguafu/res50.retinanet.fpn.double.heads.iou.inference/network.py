...
from det_opr.loss_opr import softmax_loss, smooth_l1_loss_retina, sigmoid_cross_entropy_retina, iou_l1_loss
from det_opr.utils import get_padded_tensor
from det_opr.rpn_anchor_target_opr import rpn_anchor_target_opr
...
class Network(M.Module):
    ...
    def _recover_dtboxes(self, anchors_list, rpn_cls_list, rpn_bbox_list, rpn_iou_list):
        ...
        anchors = F.broadcast_to(F.expand_dims(all_anchors, 1), (n, 2, c)).reshape(-1, c)
...
class RetinaNetCriteriaV2(M.Module):
    ...
    def anchor_iou_target_opr(self, boxes, im_info, all_anchors,
        ...
        for i in range(n):
            ...
            anchors = F.broadcast_to(F.expand_dims(all_anchors, 1), (an, 2, ac)).reshape(-1, ac)
            ...
            overlaps = overlaps.reshape(-1, 2, overlaps.shape[1]).transpose(1, 0, 2)
    ...
    def forward(self, pred_cls_list, rpn_num_prob_list, pred_reg_list,
        ...
        ignores = b - F.equal(a, 0).astype(np.float32) * F.equal(b, 0).astype(np.float32)
...
class RetinaNetHead(M.Module):
    ...
    def forward(self, features):
        ...
        pred_cls_list = [
            _.transpose(0, 2, 3, 1).reshape(_.shape[0], -1, 2*(config.num_classes-1))
            for _ in cls_prob_list]
        pred_reg_list = [
            _.transpose(0, 2, 3, 1).reshape(_.shape[0], -1, 4*2)
